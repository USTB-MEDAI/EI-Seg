...
def checkpoint_save_log(network,fold_total,fold_num,save_path, dataset):
    os.makedirs(os.path.abspath("./CrossValidationLogger"),exist_ok=True)
    csv_path = os.path.abspath(os.path.join("./CrossValidationLogger", f"{network}-{dataset}-{fold_total}.csv"))
    import pandas as pd
    if not os.path.exists(csv_path):
        df = pd.DataFrame(columns=['fold', 'save_path'])
    else:
        df = pd.read_csv(csv_path)
    if 'fold' not in df.columns or 'save_path' not in df.columns:
        df = pd.DataFrame(columns=['fold', 'save_path'])
    
    if fold_num not in df['fold'].values:
        new_row = pd.DataFrame({'fold': [fold_num], 'save_path': [save_path]})
        df = pd.concat([df, new_row], ignore_index=True)
    else:
        df.loc[df['fold'] == fold_num, 'save_path'] = save_path
    df.to_csv(csv_path, index=False)

def delete_old_ckpt(hydra_path, fold_num):
    import glob
    ckpt_pattern = os.path.join(hydra_path, f"best_checkpoint*fold{fold_num}*.pt")
...
def train(model, config, logger, train_loader, val_loader, fold_num, i, progress):
    ...
    epoch_tqdm = progress.add_task(description=f"[red]Fold{fold_num} epoch progress", total=epochs)
    batch_tqdm = progress.add_task(description=f"[blue]Fold{fold_num} batch progress", total=len(train_loader))
    val_tqdm = progress.add_task(description=f"[green]Fold{fold_num} val progress", total=len(val_loader))
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            with torch.autograd.set_detect_anomaly(True):
                ...
                if config.network == 'ei-seg-ablation' and config.Ablation_setting == '3d_only':
                    ...
                    # loss = seg_2dloss + seg_3dloss + decouple_loss + dec4_loss + dec3_loss + dec2_loss + dec1_loss
            ...
            writer.add_scalar(f"Training/fold{fold_num}/Loss", loss.item(), iteration)
            # writer.add_scalar('Training/recall', recall, iteration)
            # writer.add_scalar('Training/specificity', specificity, iteration)
            writer.add_scalar(f"Training/fold{fold_num}/dice", dice, iteration)
        ...
        torch.save(
            ...
            os.path.join(config.hydra_path, f"fold{fold_num}-{config.latest_checkpoint_file}"),
        )
        ...
        #             "epoch": epoch,
        #         },
        #         os.path.join(config.output_dir, f"checkpoint_{epoch:04d}-fold{fold_num}.pt"),
        #     )
        # ? validation
        if epoch % config.val_per_n_epochs == 0:
            
            dice_meter = AverageMeter()
            for i, batch in enumerate(val_loader):
                progress.update(val_tqdm, completed=i + 1)
                with torch.no_grad():
                    x = batch['source'][tio.DATA]
                    gt = batch['gt'][tio.DATA]
                    ...
                    dice= metric(gt.cpu(), mask.cpu(), 0)
                    dice_meter.update(dice, x.size(0))
                    logger.info(f"Validation fold {fold_num} batch {i} dice: {dice}")
                
            dice = dice_meter.avg
            logger.info(f"Validation fold {fold_num} epoch {epoch} dice: {dice}")
            writer.add_scalar(f"Validation-fold{fold_num}/dice", dice, epoch)
            if dice > val_dice_meter or epoch // config.val_per_n_epochs == 1:
                val_dice_meter = dice
                delete_old_ckpt(config.hydra_path, fold_num)
                save_path = os.path.join(config.hydra_path, f"best_checkpoint_epoch{epoch:03d}-fold{fold_num}-dice{dice:.4f}.pt")
                torch.save(
                    {
                        "model": model.state_dict(),
                        "optim": optimizer.state_dict(),
                ...
                checkpoint_save_log(config.network,config.cycle_val_fold,fold_num,save_path, config.dataset)
    writer.close()
...
def main(config):
    ...
    if config.debug_mode:
        # config.samples_per_volume = 1
        config.val_per_n_epochs = 1
    if not config.cycle_val:
        raise ValueError("Cycle validation is not enabled in the configuration.")
    ...
    #     model = UNETR(img_shape=config.patch_size, input_dim=config.in_classes, output_dim=config.out_classes)
    ...
    cvlogpath = os.path.abspath(os.path.join('./CrossValidationLogger', f'{config.network}-{config.cycle_val_fold}.csv'))
    if os.path.exists(cvlogpath):
        import shutil
        cvlog_backup_path = os.path.abspath(os.path.join('./CrossValidationLogger', f'{config.network}-{config.cycle_val_fold}-backup.csv'))
        shutil.copy(cvlogpath, cvlog_backup_path)
        logger.info(f"backup cvlog to {cvlog_backup_path}")
    
    from dataloader import Dataset_CycleVal
    
    for i,fold in enumerate(config.cycle_req_list):
        logger.info(f"Training fold {fold} start")
        model.apply(weights_init_normal(config.init_type))
        logger.info(f"fold {fold} model initialized with {config.init_type} initialization")
        
        dataset = Dataset_CycleVal(config, config.cycle_val_fold, fold)
        
        logger.info(f"val left limit: {dataset.val_l}, val right limit: {dataset.val_r}")
        train_loader = tio.SubjectsLoader(
            dataset.queue_dataset_train,
            batch_size=config.batch_size,
            shuffle=False,
        )
        val_loader = tio.SubjectsLoader(
            dataset.queue_dataset_val,
            batch_size=config.batch_size,
            shuffle=False,
        )

        train(model, config, logger, train_loader, val_loader, fold, i, progress)
        
        logger.info(f"Training fold {fold} end")
