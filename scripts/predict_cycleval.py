...
def predict(model, conf, logger, fold, index, progress):
    ...
    file_tqdm = progress.add_task(f"[red]Fold{fold} Predicting file", total=len(dataset))
    ...
    for i, item in enumerate(dataset):
        ...
        if i == 0:
            batch_tqdm = progress.add_task(f"[blue]Fold{fold} file batch", total=len(patch_loader))
        else:
        ...
        with torch.no_grad():
            ...
            # * save pred mhd file
            save_mhd(conf, pred_t, affine, i, fold)
    ...
    save_csv(conf,dice_ls, cldice_ls, precision_ls, recall_ls, hs95_ls, fold)
    if conf.network == "ei-seg" and conf.see_3d:
        save_csv(conf,dice_ls_3d, cldice_ls_3d, precision_ls_3d, recall_ls_3d, hs95_ls_3d, fold, is_3d=True)
...
def save_csv(conf, dice_ls, cldice_ls, precision_ls, recall_ls, hs95_ls, fold, is_3d=False):
    import pandas as pd
    ...
    if is_3d:
        save_path = os.path.join(conf.hydra_path, 'metrics', f"fold{fold}_3d.csv")
    else:
        save_path = os.path.join(conf.hydra_path, 'metrics', f"fold{fold}.csv")
    os.makedirs(os.path.join(conf.hydra_path, 'metrics'), exist_ok=True)
    df.to_csv(save_path, index=False)


def save_mhd(conf, pred, affine, index, fold):
    save_base = os.path.join(conf.hydra_path, 'pred_file', f"fold{fold}")
...
def main(conf):
    conf = conf["config"]
    if not conf.predict.cycle_val:
        raise NotImplementedError("predict.cycle_val is not enabled in the configuration. Please enable it or use predict.py to predict the model")
    ...
    for i,fold in enumerate(conf.predict.cycle_req_list):
        logger.info(f"Predicting fold {fold} start")
        if conf.predict.ckpt_from_csv:
            import pandas as pd
            df = pd.read_csv(f"./CrossValidationLogger/{conf.network}-{conf.dataset}-{conf.predict.cycle_val_fold}.csv")
            ckpt_path = df.loc[df['fold'] == fold, 'save_path'].iloc[0]
        else:
            ckpt_path = getattr(conf.predict, f'fold_{fold}_ckpt')
            
        logger.info(f"load ckpt from {ckpt_path}")
        model.load_state_dict(torch.load(ckpt_path)['model'])
        predict(model, conf, logger, fold, i, progress)
        logger.info(f"Predicting fold {fold} end")
