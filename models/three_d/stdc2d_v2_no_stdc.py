...
class ProjectionSegNet(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, init_features=32, tsne_mode=False, tsne_layer='dec4') -> None:
        super().__init__()
        self.img_size = 64
        ...
        self.encoder = StdcEncoder(in_channels, init_features, no_stdc=True)
    ...
    def forward(self, inputs):
        # * 3d 2d Path
        xy_planes, xz_planes, yz_planes, masks, out_3d_list, stack_list = self.encoder(inputs)
...
class SeperateSegNet(nn.Module):
    ...
    def __init__(self, in_channels=1,out_channels=1, init_features=32):
        super().__init__()
        self.init_features = init_features
        self.encoder = StdcEncoder(in_channels, init_features, seperate=True, no_stdc=True)
        self.seg_path = UnetDecoder(init_features, out_channels, dim='3d')
